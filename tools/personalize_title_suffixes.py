"""Body-evidenced title suffixes for site8; does not regenerate manuscripts."""
...
BASELINE_COMMIT = "31676c5c18f4825d647cfe447d0baf413609a4ca"
EXPECTED_TARGETS = 8928
EXPECTED_HTML = 8933
EXPECTED_SITEMAP = 8545
EXPECTED_NOINDEX = 388
...
OPERATIONAL = re.compile(r"사물함|환불|등록번호|등록 자료|제휴|센터 위치|제공 주소|제공된.{0,8}학교|학교 정보|학교 목록|주소는|주소가|기준 주소|제공된 수업 주소|센터 자료|센터정보|등록 제|상담 신청|전화상담|문자상담|학원혜택|학원이벤트|학원오리엔테이션|교습비|총 납부액|등록 전|운영 중인 반|방문 센터|수업 가능|확인된.{0,20}학교|제공 수업학교|가능 여부|가능 학년")
...
def learning_blocks(source, kind):
    ...
    for block in parser.blocks:
        ...
        if classes & {"center-facts-panel", "academy-side-card", "review-card", "school-card", "subject-info-card"}:
            continue
        weight = 0
        if kind == "subject":
            if "manuscript-article" in classes:
                if "section-01" in ids or "manuscript-intro" in classes:
                    weight = 6 if block["tag"] == "p" else 7
                else:
                    weight = 2.8 if block["tag"] == "p" else 1.8
            elif "subject-quick-answer" in classes and block["tag"] == "p":
                weight = 6
            elif "academy-hero-main" in classes and block["tag"] == "p":
                weight = 1.5
        elif kind == "national":
            if "answer-box" in classes:
...
def hub_copy(source, terms):
    ...
    return next((b["text"] for b in parser.blocks
                 if all(term in b["text"] for term in terms) and any(
                     "academy-hero-main" in (a.get("class", "") or "").split()
...
def read_page(path):
    ...
    # Two older national categories use root-relative canonical values.
    # Preserve those bytes and resolve only the URL used for HTTP verification.
